...
var CheckOpen = 0;
function CheckOpenQuiz() {
  db.where('GroupQuiz','==',sGroupQuiz)
  .where('QuizDate','==',today).get().then((snapshot)=> {
    ...
    snapshot.forEach(doc=> {
      ...
      if(doc.data().QuizTypeQuestion=="1") {
        ...
        if(doc.data().QuizQuestion!=null) {
          $("#DisplayQuestion").html("<div><img src='"+ doc.data().QuizImg +"' class='imggame' style='max-width:370px;width:90%;'><div class='txt-qq'>"+doc.data().QuizQuestion+"</div></div>");
        } else {
        ...
        if(doc.data().QuizVDO!=null) {
          $("#DisplayQuestion").html("<div>"+ doc.data().QuizVDO +"<div class='txt-qq'>"+doc.data().QuizQuestion+"</div></div>");
        } else {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      if(CheckAddEdit==2) {
        ...
        //$("#DisplayQuestion").html("<div class='txt-qq'>คุณได้เข้าร่วมกิจกรรมนี้ไปแล้วในวันนี้<div>คุณทำคะแนนได้ : <span class='txt-qqq'>"+ LastScore +"</span> คะแนน111</div></div>");
      } else {
...
function SaveMyScorePoint() {
  ...
  //sRewardsXP = parseFloat(sRewardsXP).toFixed(2)+parseFloat(YourScore).toFixed(2);
  //sRewardsRP = parseFloat(sRewardsRP).toFixed(2)+parseFloat(YourScore).toFixed(2);
  //alert(YourScore+"---"+sRewardsXP+"---"+sRewardsRP);
  //if(sUserLevel==0) { sUserLevel = 1; }
...
function ClearQuiz() {
  ...
  if(YourScore!=0) {
    ...
    var str2 = "";
    str2 += '<div><img src="./img/false.png" width="60px;"></div>';
...
function ChangeNow() {
  ...
  if(CheckAddEdit==2) { 
    ...
    dbScorePoint.doc(EidScorePoint).update({
      ...
      //RewardsXP : parseFloat(sRewardsXP-YourScore+newScore).toFixed(2),
      //RewardsRP : parseFloat(sRewardsRP-YourScore+newScore).toFixed(2)
    });
